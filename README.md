...
Export the nintent mDNS bootstrap inventory from Nautobot:
...
`make pipeline` (bootstrap export, nodeutils collect/ingest, production export)
...
- `export_nintent_hosts_intent.yml` runs nintent's Nautobot `Export Ansible Hosts Intent` Job, downloads the generated schema `2.0` `hosts_intent.yml` JobResult file, validates a staged copy with `ansible-inventory --list`, and only then atomically replaces `inventories/generated/hosts_intent.yml`, preserving the previous file if validation fails. This generated inventory is the mDNS-only bootstrap inventory for name-reserved nodes; it carries no service groups or desired `host_os`, and is not the detailed canonical inventory.
- `export_nintent_production.yml` serializes `vars/deployment_profiles.yml` to canonical JSON with its SHA-256 digest (via the shared `playbooks/tasks/nintent_serialize_deployment_profiles.yml`), runs nintent's Nautobot `Export Production Inventory` Job with both values, downloads the generated schema `1.0` `production.yml` plus its generation-addressed JSON report, verifies they share a matching generation id, profile digest, and schema, validates a staged copy with `ansible-inventory --list`, installs the immutable report under `inventories/generated/production.reports/<generation_id>.json`, and only then atomically replaces `inventories/generated/production.yml`, preserving the previous file if any step fails. This joined desired/actual inventory is the canonical production inventory; the bootstrap `hosts_intent.yml` remains separate. Run it after a nodeutils collection and ingest so realized actual facts exist.
- `sync_nintent_deployment_profiles.yml` serializes `vars/deployment_profiles.yml` with the same shared `playbooks/tasks/nintent_serialize_deployment_profiles.yml` and runs nintent's Nautobot `Sync Deployment Profiles` Job with the resulting `deployment_profiles_json` + `deployment_profiles_digest`. This projects the Ansible-owned profile map into nintent's read-only, digest-keyed projection so the Quick Service Placement UI can offer profile choices and config schemas; Ansible stays the authoritative owner and production export still revalidates the map. Run it after editing `vars/deployment_profiles.yml`. It does not download any artifact.
- The export and sync playbooks share `playbooks/tasks/nintent_serialize_deployment_profiles.yml` for the canonical deployment-profile JSON + digest byte contract, and `playbooks/tasks/nautobot_run_job.yml` (plus `nautobot_download_file.yml` for the export) for JobResult polling and FileProxy download, instead of duplicating that code. `make pipeline` runs the full refresh: `bootstrap-inventory`, `collect-ingest`, then `production-inventory`.
